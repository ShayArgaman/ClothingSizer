...
export default function ComponentsPanel({ onAdd, onTouchDragStart }: Props) {
      ...
      {COMPONENT_TEMPLATES.map((template) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] text-slate-500 mt-0.5 leading-tight hidden sm:block">{template.description}</div>
          </div>
        </div>
      ))}
    </div>
  ...
  // Mobile: render as full-width content (no aside)
  if (isMobileSheet) {
    return <div className="w-full pb-2">{itemsGrid}</div>
  }
